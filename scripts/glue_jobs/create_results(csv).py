...
def main():
    ...
    up_features = glueContext.create_dynamic_frame_from_options(connection_type = "parquet", connection_options = {"paths": ["s3://imba-aws/features/up_features_db/"]})
    prd_features = glueContext.create_dynamic_frame_from_options(connection_type = "parquet", connection_options = {"paths": ["s3://imba-aws/features/prd_features_db/"]})
    ...
    # save csv
    df_spark.repartition(1).write.mode('overwrite').format('csv').save("s3://imba-aws/output", header = 'true')
